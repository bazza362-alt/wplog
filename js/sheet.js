...
export const Sheet = {
    ...
    _renderOffensiveActions(game) {
        const data = Game.buildOffensiveActionsSummary(game);
        const section = document.createElement("div");
        section.className = "sheet-section sheet-actions-section";
        section.innerHTML = `<div class="sheet-section-title">Offensive Actions</div>`;

        const hasAny = data.W.actions > 0 || data.D.actions > 0;
        if (!hasAny) {
            const empty = document.createElement("p");
            empty.className = "sheet-empty";
            empty.textContent = "No offensive actions recorded.";
            section.appendChild(empty);
            return section;
        }
        ...
        table.innerHTML = `
      <thead>
        <tr><th>Team</th><th>Actions</th><th>Goals</th><th>Efficiency</th></tr>
      </thead>
    `;
        ...
        for (const t of teams) {
            ...
            const tr = document.createElement("tr");
            tr.innerHTML = `
        <td class="sheet-team-cell">${t.label}</td>
        <td>${d.actions}</td>
        <td>${d.goals}</td>
        <td>${d.efficiency === null ? "-" : d.efficiency + "%"}</td>
      `;
            tbody.appendChild(tr);
        }
